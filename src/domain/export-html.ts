...
function renderValue(field: WorkflowField, value: string): string {
  ...
  if (empty) {
    if (field.displayFormat === 'bullet-list') return '<ul class="value bullet-list empty" data-value="true" data-empty="true" data-slot="future-model" aria-label="空白项目列表"></ul>'
    if (field.displayFormat === 'steps') return '<ol class="value steps empty" data-value="true" data-empty="true" data-slot="future-model" aria-label="空白步骤列表"></ol>'
    if (field.displayFormat === 'checklist') return '<ul class="value checklist empty" data-value="true" data-empty="true" data-slot="future-model" aria-label="空白检查清单"></ul>'
    return '<div class="value empty" data-value="true" data-empty="true" data-slot="future-model" aria-label="空白内容槽"></div>'
  }
  const lines = valueLines(value)
  if (field.displayFormat === 'bullet-list') return `<ul class="value bullet-list" data-value="true">${lines.map((line) => `<li>${renderProseText(line.replace(/^[-*]\s*/, ''))}</li>`).join('')}</ul>`
